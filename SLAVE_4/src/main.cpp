...
typedef struct struc_message{
  float VelAire;
  float RPM;
  float uvintensity;//cambiar por i minuscula
} struct_message;
...
float averageAnalogRead(float pinToRead)
{
  ...
  float pendiente = (2.43/3);
  float ecuacion = (pendiente*pinToRead);
  for(int x = 0 ; x < numberOfReadings ; x++)
    runningValue += ecuacion;
...
float mapfloat(float x, float in_min, float in_max, float out_min, float out_max)
{
  ...
  int ak = x;
  if(ak<1000){ak=ak*-1;}
  return (((pendiente*ak)-(pendiente*in_max)+ (out_max))/1000);
}
...
void loop()
{
  ...
  int uvLevel =averageAnalogRead(analogRead(UVOUT));// averageAnalogRead(analogRead(UVOUT)-10)/1000;
  if(uvLevel<1000){
    uvIntensity = mapfloat(1000, 0.99*1000, 3*1000, 0*1000, 15*1000);}
  else{
    uvIntensity = mapfloat(uvLevel, 0.99*1000, 2.8*1000, 0*1000, 15*1000); 
  } 
  Serial.printf("\nRPM: %i rpm \n",Rpm);
